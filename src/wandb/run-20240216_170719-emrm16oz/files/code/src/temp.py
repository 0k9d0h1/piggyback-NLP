...
class PiggybackStrategy(SupervisedTemplate):
    ...
    def forward(self):
        self.input_ids = self.mb_x[:, 0, :].to('cuda').type(torch.IntTensor)
        self.original_input_ids = self.mb_x[:, 1, :].to(
            'cuda').type(torch.IntTensor)
        self.token_type_ids = self.mb_x[:, 2, :].to(
            'cuda').type(torch.IntTensor)
        self.attention_mask = self.mb_x[:, 3, :].to(
            'cuda').type(torch.IntTensor)
